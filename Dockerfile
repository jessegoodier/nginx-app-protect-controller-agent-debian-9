...
RUN set -ex \
  ...
  python3 python3-minimal python3.5 python3.5-minimal binutils net-tools wget curl \
  && \
...
RUN set -ex \
  && printf "deb https://app-protect-sigs.nginx.com/debian stretch nginx-plus\n" > /etc/apt/sources.list.d/app-protect-sigs.list \
  && wget https://nginx.org/keys/app-protect-sigs.key && sudo apt-key add app-protect-sigs.key \
  && wget -P /etc/apt/apt.conf.d https://cs.nginx.com/static/files/90app-protect-sigs

#install app protect
RUN set -ex \ 
  && apt-get update && apt-get install -y  app-protect app-protect-attack-signatures app-protect-threat-campaigns \
